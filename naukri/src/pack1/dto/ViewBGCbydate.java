...
public class ViewBGCbydate {
	static Statement st = null;
	static ResultSet rs= null;
	GetCon gc;
	ArrayList<BGC> al=new ArrayList<BGC>();
	public ArrayList viewBGCAd(String stdate,String enddate)
	{
	int i=1;	
		try
		{
	...
	System.out.println("hi in viewbgc dto"+stdate);
	rs=st.executeQuery("select * from bgc where stdate between '"+stdate+"' and '"+enddate+"'");
	while(rs.next())
	{
		BGC bg=new BGC();
